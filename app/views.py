...
def signup_view(request):
    if request.method=='POST':
        ...
        form = SignUpForm()
    today=datetime.now()
    return render(request,'index.html',{'today1':today},{'form':form})
    ...
    if user:
        if request.method=='GET':
            ...
            if form.is_valid():
                ...
                post = PostModel(user=user, immage=image, caption=caption)
                post.save()
                path = str(BASE_DIR + post.image.url)
                client = ImgurClient('061fbca36570a61', '7d9841c62e75b77a4592391463c09747496e0cd7')
...
def comment_view(request):
    ...
    if user and request.method == 'POST':
        ...
        if form.is_valid():
            post_id = form.cleaned_data.get('comment_text')
    ...
    #return render(request, 'index.html', {'form':form, 'today' : date, 'user_name':'Anirudh Singla'})
